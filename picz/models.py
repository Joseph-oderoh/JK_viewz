...
class Images(models.Model):
    ...
    '''
    image_link = models.ImageField(upload_to='media/',default='DEFAULT VALUE')
    ...
    @classmethod
    def search_image(cls, catg):
        '''
        method to search images by category
        '''
        retrieved = cls.objects.filter(category__name__contains=catg) #images assoc w/ this catg
...
class Categories(models.Model):
    ...
    @classmethod
    def update_category(cls, search_term , new_catg):
        '''
        method to update a category
        '''
        try:
            to_update = Categories.objects.get(name = search_term)
            to_update.name = new_catg
            to_update.save()
            return to_update
